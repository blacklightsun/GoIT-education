with monthly_revenue as (
    select date(date_trunc('month', payment_date)) as payment_month,
        user_id,
        game_name,
...
revenue_lag_lead_months as (
    select *,
        ...
        lag(payment_month) over(
            partition by user_id
            order by payment_month
        ) as previous_paid_month, -- визначаємо попердній місяць оплати, навіть з пропуском
        lead(payment_month) over(
            partition by user_id
            order by payment_month
        ) as next_paid_month --only next month, but no revenue of next month
...
revenue_metrics as (
    ...
    where previous_paid_month != previous_claendar_month
        and previous_paid_month is not null --good
    union all
    select payment_month,
        user_id,
        game_name,
        - total_revenue as revenue_amount, --рахуємо ні різницю, як зміну, а усю суму ревенью - це помилка!!!!! 
...
where user_id = '2FYEYdoJMUOtbJ6TruruWA=='